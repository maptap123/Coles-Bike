...
function routeStartsAt(route, routeStart) {
  const first = route[0];
  ...
  return (
    Math.abs(Number(first.lat) - Number(routeStart.lat)) < 0.00001 &&
    Math.abs(Number(first.lng) - Number(routeStart.lng)) < 0.00001 &&
...
function applyRouteStart(progress) {
  ...
  const alreadyApplied = progress.routeStartApplied === startKey;

  if (alreadyApplied && routeStartsAt(route, routeStart)) {
    return progress;
  }
  ...
  return {
    ...
    current: alreadyApplied
      ? progress.current
      : rebasePointMiles(progress.current, startMiles, connectorMiles),
    actualPath: Array.isArray(progress.actualPath)
      ? progress.actualPath.map((point) =>
          alreadyApplied ? point : rebasePointMiles(point, startMiles, connectorMiles),
        )
      : progress.actualPath,
    route: rebasedRoute,
...
export async function writeProgress(progress) {
  ...
  const { route, ...storedProgress } = progress;
